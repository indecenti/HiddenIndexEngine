...
def _ps(value: int) -> int:
    """A properties panel offset at the current UI scale.

    The panel stacks small rows whose spacing was written for the font at
    scale 1.0; the font grows with the UI scale and the spacing has to grow
    with it, or every label lands on the field under it.
    """
...
class RenderPanelsMixin:
    ...
    def _prop_field_metrics(self, sample: str = "-0000.00") -> tuple:
        """(box width, field height) of a number row, at this UI scale.

        The box used to be 60x22 whatever the font was, so a larger UI scale
        turned every value into "20...". `sample` is the longest value the row
        is expected to show.
        """
        text_w, text_h = _text_wh(sample, "mono")
        return max(60, text_w + 14), max(22, text_h + 6)
    ...
    def _r_props(self, rx0, h):
        ...
        if has_fx_sel:
            ...
            self.screen.blit(hdr, (rx0+8, y)); y += _ps(20)
            pygame.draw.line(self.screen, BORDER, (rx0, y), (rx0+self.panel_r_w, y)); y += _ps(8)
            ...
            self.screen.blit(sv, (rx0+140, y)); y += _ps(24)

            # Background
            bg_name = self.scene_data.get("background", "background.jpg")
            s = _txt(self._TR("prop_background"), "sm", TXT_DIM)
            self.screen.blit(s, (rx0+8, y)); y += _ps(20)
            _draw_text(self.screen, bg_name, "sm", TXT, rx0+8, y, self.panel_r_w-16); y += _ps(20)
            ...
            self.screen.blit(hdr2, (rx0+8, y)); y += _ps(20)
            ...
            self.screen.blit(hdr3, (rx0+8, y)); y += _ps(20)
            ...
            self.screen.blit(hdr5, (rx0+8, y)); y += _ps(20)
            ...
            self.screen.blit(hdr4, (rx0+8, y)); y += _ps(20)
            ...
            # Posizione e Scala
            _draw_text(self.screen, self._TR("prop_transform_hdr"), "sm", ACCENT, rx0+12, y); y += _ps(26)
            ...
            def _prop_row_obj(label, key):
                ...
                box_w, field_h = self._prop_field_metrics()
                box_y = y + label_h + 3
                ...
                box_r = pygame.Rect(rx0 + 12, box_y, box_w, field_h)
                ...
                slider_rect = (slider_x, box_y + 1, slider_w, field_h - 2)
                ...
                self._prop_num_hitboxes(spec, (12, box_y, box_w, field_h),
                                        (12 + box_w + 10, box_y + 1, slider_w,
                                         field_h - 2))
                y = box_y + field_h + _ps(8)
            ...
            y += _ps(16)
            _draw_text(self.screen, self._TR("prop_visual_hdr"), "sm", ACCENT, rx0+12, y); y += _ps(26)
            ...
            pygame.draw.line(self.screen, BORDER, (rx0 + 12, y), (rx0+self.panel_r_w - 12, y))
            y += _ps(16)
            _draw_text(self.screen, self._TR("prop_gameplay_hdr"), "sm", ACCENT, rx0+12, y); y += _ps(26)
    ...
    def _r_effect_props(self, rx0, h, idx, y):
        ...
        self.screen.blit(hdr, (rx0+12, y)); y += _ps(22)
        lbl = fx_cat.get("label", fx.get("effect_id", "?")) if fx_cat else fx.get("effect_id", "?")
        _draw_text(self.screen, lbl.upper(), "md", TXT_HI, rx0+12, y, self.panel_r_w-24); y += _ps(28)
        pygame.draw.line(self.screen, BORDER, (rx0+8, y), (rx0+self.panel_r_w-8, y)); y += _ps(16)
        ...
        def _prop_row(label, key, min_val, max_val, fmt="{:.2f}"):
            ...
            _draw_text(self.screen, label, "sm", TXT_DIM, rx0+12, y,
                       self.panel_r_w - 24)
            y += _text_wh("Ag", "sm")[1] + 3

            box_w, field_h = self._prop_field_metrics()
            box_r = pygame.Rect(rx0+12, y, box_w, field_h)
            ...
            self._set_hbox_fx(f"box_{key}", 12, y, box_w, field_h)

            slider_x = rx0 + 12 + box_w + 10
            slider_w = self.panel_r_w - (box_w + 24 + 12)
            slider_rect = (slider_x, y + 1, slider_w, field_h - 2)
            ...
            self._set_hbox_fx(f"slider_{key}", 12 + box_w + 10, y + 1, slider_w,
                              field_h - 2)
            y += field_h + _ps(8)
        ...
        if fx.get("type") == "bubble_tip":
            ...
            pygame.draw.line(self.screen, BORDER, (rx0+12, y), (rx0+self.panel_r_w-12, y))
            y += _ps(16)
            _draw_text(self.screen, self._TR("prop_text_props_hdr"), "sm", ACCENT, rx0+12, y); y += _ps(26)
